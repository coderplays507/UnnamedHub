...
-- Content Frame (hides when minimized)
...
-- Create all toggles
createToggle("Aimbot", 10, function(enabled) aimbotEnabled = enabled end)
createToggle("ESP", 50, function(enabled) espEnabled = enabled; toggleESP() end)
createToggle("No Recoil", 90, function(enabled) noRecoilEnabled = enabled; toggleNoRecoil() end)
createToggle("Speed Hack", 130, function(enabled) speedHackEnabled = enabled; toggleSpeedHack() end)
createToggle("Fly", 170, function(enabled) flyEnabled = enabled; toggleFly() end)

-- Kill Button (Teleport to void)
createButton("💀 KILL", 210, function()
    pcall(function()
        local character = LocalPlayer.Character
        if character then
